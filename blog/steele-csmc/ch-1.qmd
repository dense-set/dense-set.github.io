...
### Why not three or more?
  ...
  1. $\sum_i a_i b_i c_i\le \bigl( \sum_i a_i^2 \bigr)^{1/2} \bigl( \sum_i b_i^4 \bigr)^{1/4} \bigl( \sum_i c_i^4 \bigr)^{1/4}$
  2. $\sum_i a_i b_i c_i\le \sqrt{\sum_i a_i^2}\, \sqrt{\sum_i b_i^2} \, \sqrt{\sum_i c_i^2}$
:::
...
:::{.callout-tip collapse="true"}
#### A solution
  We have $\bigl( \sum_i a_i b_i c_i \bigr)^4$ $\le$ $\bigl( \sum_i a_i^2 \bigr)^2  \bigl( \sum_i b_i^2 c_i^2 \bigr)^2$	$\le$ $\bigl( \sum_i a_i^2 \bigr)^2  \bigl( \sum_i b_i^4 \bigr)  \bigl( \sum_i c_i^4 \bigr)$ $\le$ $\bigl( \sum_i a_i^2 \bigr)^2  \bigl( \sum_i b_i^2 \bigr)^2 \bigl( \sum_i c_i^2 \bigr)^2$ where the last inequality follows because $\sum_i \alpha_i^2\le (\sum_i \alpha_i)^2$ for any $\alpha_1, \ldots, \alpha_n\ge 0$.
:::
...
:::{.callout-tip collapse="true"}
#### A solution 
  This is equivalent to showing that $n\sum_i (p_i + 1/p_i)^2\ge (n^2 + 1)^2$. Indeed, $n\sum_i (p_i + 1/p_i)^2\ge \bigl(\sum_i (p_i +1/p_i)\bigr)^2\which=(1 + \sum_i 1/p_i)^2\which\ge (1 + n^2)^2$, where the last inequality follows from @lem-1_over_p_sq below. For equality, "$\Leftarrow$" is clear. For the converse, note that all the inequalities must be equalities so that $p_i + 1/p_i$ is independent of $i$ which implies that $p_i$'s are all equal.
:::
...
3. p. 228: In the solution to Ex 7, "$\iprd{x, y} = 5x_1 y_1 + x_1 y_2 + x_2 y_1 + \cancel{3y_2^2} \mathcorr{3x_2 y_2}$" and "$5z^2 + \cancel{3z}\mathcorr{2z} + 3 = 0$".